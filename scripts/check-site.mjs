 ...
 * docs/ は scripts/build.mjs が毎日作り直し、そのまま GitHub Pages で配られるので、docs/ を直接見る。
 *
 ...
 *      docs/ のファイルに解決できる
 *   2. / で始まる参照が無い (旧 URL の /OmekaS/ 配下でもホスト直下でも同じに動くよう、相対で書く)
 *   3. 旧ホスト (github.io/OmekaS) を指す文字列が残っていない
 *   4. robots.txt の Sitemap と sitemap.xml の各 URL が実在するページを指す
 */
import { readFileSync, readdirSync, statSync, existsSync } from 'node:fs';
import { join, relative, posix } from 'node:path';
import { SITE_URL } from './build.mjs';

const DOCS = new URL('../docs/', import.meta.url).pathname;
const siteUrl = SITE_URL.replace(/\/$/, '');
const FORBIDDEN = ['nakamura196.github.io/OmekaS'];
...
function checkRef(ref, file) {
  ...
  // index.html の中のクライアント側テンプレート (`${esc(t.url)}` など)。値は実行時に入る外部 URL。
